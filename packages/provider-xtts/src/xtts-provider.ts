...
import { XttsBackendAdapter } from "./adapter.js";

export interface XttsProviderConfig {
  adapter: XttsBackendAdapter;
  defaultVoice?: string;
}
...
export class XttsProvider implements TTSProvider {
  ...
  private adapter: XttsBackendAdapter;
  private defaultVoice: string;
  private voiceCache: VoiceInfo[] | null = null;

  constructor(config: XttsProviderConfig) {
    this.adapter = config.adapter;
    this.defaultVoice = config.defaultVoice ?? "xtts/multilingual";
  }
